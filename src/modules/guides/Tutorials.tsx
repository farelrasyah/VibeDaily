...
export default function Tutorials() {
  ...
  const [tutorialArticles, setTutorialArticles] = useState<GuideArticle[]>([])
  ...
  useEffect(() => {
    const fetchTutorials = async () => {
      try {
        setIsLoading(true)
        setError(null)
        const articles = await guidesService.getTutorials(20)
        setTutorialArticles(articles)
      } catch (err) {
        ...
        <div className="max-w-[1440px] mx-auto">
          ...
          <header className="mb-8">
            ...
            <p className="text-lg text-slate-600 max-w-3xl">
              Comprehensive, hands-on tutorials designed to take you from beginner to expert.
              Each tutorial includes practical examples, code snippets, and step-by-step explanations
          ...
          {isLoading && (
            <div className="flex items-center justify-center py-12">
              <div className="flex items-center gap-3">
                ...
                <span className="text-slate-600">Loading tutorials...</span>
              </div>
            </div>
          )}
          ...
          {error && (
            <div className="text-center py-12">
              <div className="max-w-md mx-auto">
                ...
                <h3 className="text-lg font-medium text-gray-900 mb-2">Error loading tutorials</h3>
          ...
          {!isLoading && !error && (
            ...
            {/* Difficulty Filter */}
            <div>
              <h3 className="text-sm font-medium text-slate-700 mb-2">Difficulty Level</h3>
              ...
              >
                <div className="mb-4">
                  <div className="flex items-center gap-2 mb-3">
                    <span className={`px-2 py-1 text-xs font-medium rounded-full border ${getDifficultyColor(article.difficulty || 'Intermediate')}`}>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1">
                    <div className={`w-2 h-2 rounded-full ${
                      ...
                      article.difficulty === 'Intermediate' ? 'bg-yellow-500' : 'bg-red-500'
                    }`}></div>
                    <span className="text-xs text-slate-500 capitalize">{(article.difficulty || 'Intermediate').toLowerCase()}</span>
                  </div>
                  <div className="flex items-center text-violet-600 group-hover:translate-x-1 transition-transform">
                    <span className="text-sm font-medium mr-1">Start Tutorial</span>
          ...
          {filteredArticles.length === 0 && (
            <div className="text-center py-12">
              <div className="max-w-md mx-auto">
                ...
                <p className="text-gray-500">Try adjusting your filters to see more tutorials.</p>
              </div>
            </div>
          )}
            </>
