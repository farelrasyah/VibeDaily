...
export default function LikedArticlesPage() {
  ...
  const { articles, loading, error, refetch } = useLikedArticles()
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center justify-between mb-12">
          ...
          <div className="text-center flex-1 mx-8">
            ...
            <p className="text-slate-600 text-lg font-medium">Your favorite stories, beautifully curated</p>
          </div>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20">
            <div className="relative">
              ...
              <div className="absolute inset-0 w-16 h-16 border-4 border-pink-200 border-t-pink-500 rounded-full animate-spin animation-delay-150"></div>
            </div>
            <span className="text-slate-600 mt-6 font-medium text-lg">Curating your liked articles...</span>
          </div>
        ) : error ? (
          ...
          <div className="text-center py-20">
            ...
            <h3 className="text-2xl font-bold text-slate-900 mb-4">No liked articles yet</h3>
            <p className="text-slate-600 mb-8 max-w-md mx-auto text-lg">Start your reading journey by liking articles that resonate with you!</p>
            <Link
              href="/"
              className="px-8 py-4 bg-gradient-to-r from-violet-600 via-purple-600 to-pink-600 text-white rounded-2xl hover:from-violet-700 hover:via-purple-700 hover:to-pink-700 transition-all duration-300 shadow-lg hover:shadow-2xl hover:scale-105 font-semibold text-lg inline-block"
            >
          ...
          <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {articles.map((article, index) => (
              <article
                key={article.id}
                className="group bg-white/80 backdrop-blur-sm rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-500 overflow-hidden border border-slate-200/50 hover:border-violet-200/50 hover:scale-[1.02] hover:-translate-y-2"
                style={{
              ...
              >
                ...
                <div className="p-6">
                  <div className="flex items-center gap-3 mb-4">
                    <span className="px-3 py-1.5 bg-gradient-to-r from-red-50 to-pink-50 text-red-700 text-xs font-semibold rounded-full border border-red-100">
                  ...
                  <h3 className="text-lg font-bold text-slate-900 mb-4 group-hover:text-violet-600 transition-colors duration-300 leading-tight overflow-hidden" style={{
                    display: '-webkit-box',
                    WebkitLineClamp: 2,
                    WebkitBoxOrient: 'vertical' as const,
                    lineHeight: '1.4'
                  }}>
                    <Link href={`/article/${article.id}`} className="hover:underline decoration-violet-300 decoration-2 underline-offset-2">
                      {article.title}
                    </Link>
                  </h3>
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-slate-100">
                    <div className="flex items-center gap-2">
                      <div className="w-8 h-8 bg-gradient-to-br from-red-400 to-pink-500 rounded-xl flex items-center justify-center shadow-md">
                    ...
                    <Link
                      ...
                      className="group/btn flex items-center gap-2 text-violet-600 hover:text-violet-700 text-sm font-semibold transition-all duration-300 hover:scale-105"
                    >
                      <span>Read More</span>
